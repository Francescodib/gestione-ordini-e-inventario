...
async function testValidStatusChanges() {
  try {
    console.log('🧪 Testing VALID order status change notifications...\n');
    ...
    // 3. Create a new order (will be PENDING)
    ...
    console.log(`✅ Created order #${orderId} (status: PENDING)\n`);

    // Wait for creation notification
    await new Promise(resolve => setTimeout(resolve, 2000));

    // 4. Valid transition: PENDING → PROCESSING
    ...
    // 5. Valid transition: PROCESSING → SHIPPED
    ...
    console.log(`6. Changing order #${orderId} status: SHIPPED → DELIVERED...`);
    const statusResponse3 = await axios.put(`${API_BASE}/orders/${orderId}/status`, {
      status: 'DELIVERED'
    }, {
      headers: { Authorization: `Bearer ${token}` }
    });

    console.log('✅ Status change successful:', statusResponse3.status);
    console.log('Response:', statusResponse3.data.message);

    // Wait for final notification
    await new Promise(resolve => setTimeout(resolve, 3000));

    console.log('✅ Test completed with valid status transitions!');
    socket.disconnect();
    process.exit(0);
